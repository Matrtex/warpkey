...
2. 如果没有配置 `APP_URL`，跳过健康检查并返回成功。
3. 如果配置了 `CRON_SECRET`，请求 `${APP_URL}/api/cron`。
4. 如果没有配置 `CRON_SECRET`，退化检查公开 `${APP_URL}/api/full`。
5. 单次检查最多重试 5 次，避免上游或部署站点短暂抖动导致误报。
6. `/api/cron` 会校验 JSON 中的 `success` 字段和 `full` 数量。
7. `/api/full` 会校验 HTTP 状态码和 key 行格式。
8. 输出 full/lite 数量或 key 数量，便于在日志中快速判断抓取结果。

如果没有配置 `APP_URL`，workflow 会跳过健康检查并返回成功，避免 fork 或未部署仓库因为默认公共站点抖动而被标红。如果没有配置 `CRON_SECRET`，workflow 不会失败，而是检查公开 `/api/full` 接口。
